...
func (rf *Raft) handleVoteResult(reply RequestVoteReply) {
	...
	if rf.state == CANDIDATE && reply.VoteGranted {
		rf.voteNum += 1
		if rf.voteNum >= majority(len(rf.peers)) {
			rf.state = LEADER
...
func (rf *Raft) resetTimer() {
	...
	new_timeout := HeartbeatTime
	if rf.state != LEADER {
		new_timeout = ElectionMinTime + rand.Intn(ElectionMaxTime - ElectionMinTime)
	}
	rf.timer.Reset(time.Millisecond * time.Duration(new_timeout))
